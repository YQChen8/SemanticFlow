...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if 'pc0_valid_point_idxes' in res_dict:
            ...
            final_flow[batch['gm0']] = res_dict['flow'] + pose_flow[batch['gm0']]
            final_label[batch['gm0']] = init_label[batch['gm0']] + torch.argmax(res_dict['masks0'], dim=1).float()
        ...
        # Update or add the flow and label data
        data_dict[self.vis_name] = final_flow.cpu().detach().numpy().astype(np.float32)
        data_dict['est_label0'] = final_label.cpu().detach().numpy().astype(np.float32)
        
        # Add all batch data to data_dict
        for key, value in batch.items():
            if isinstance(value, torch.Tensor):
                data_dict[key] = value.cpu().detach().numpy().astype(np.float32)
            else:
                data_dict[key] = value
