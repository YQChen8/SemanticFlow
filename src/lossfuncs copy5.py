...
def seflowLoss(res_dict, timer=None):
    ...
    rigid_loss = rigidLoss(pc0, pc0_mask.unsqueeze(0), est_flow)
    ...
    if N >= target_num_points:
        ...
        pc0_mask_downsampled = pc0_mask.unsqueeze(0)[:, indices, :]  # 选择对应的 mask
    else:
        pc0_downsampled = pc0
        pc0_mask_downsampled = pc0_mask.unsqueeze(0)
    ...
    # 这里计算的是前背景点的分类损失
    BSMaskLoss = BinaryFocalLoss(alpha=1.0, gamma=2.0)
    bs_mask_loss = BSMaskLoss(pc0_mask, pc0_label.long())
