...
class MaskConvGRUDecoder(nn.Module):
    ...
    def __init__(self, pseudoimage_channels: int = 64, num_iters: int = 4, num_classes: int = 16):
        ...
        # self.upsample = nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=4, stride=2, padding=1)
        # 将 128 通道降维到 3 通道，适配 classifier
        self.reduction_conv = nn.Conv2d(in_channels=128, out_channels=3, kernel_size=1)
        # MaskFormer的配置
        config = MaskFormerConfig(
            hidden_size=pseudoimage_channels * 3,
            num_labels=num_classes
        )
        # 引入 MaskFormer 模型
        self.classifier = MaskFormerModel(config)
        # 添加无监督分类的线性层
        self.classifier_decoder = nn.Sequential(
            nn.Linear(1024, pseudoimage_channels // 2), 
            # nn.BatchNorm1d(pseudoimage_channels // 2),
            nn.GELU(),
            nn.Linear(pseudoimage_channels // 2, num_classes)
        )
    ...
    def forward_single(self, before_pseudoimage: torch.Tensor,
        ...
        # 分类预测
        maskformer_inputs = torch.cat([before_pseudoimage,after_pseudoimage],dim=0).unsqueeze(0)
        reduced_maskformer_inputs = self.reduction_conv(maskformer_inputs)
        # 使用 MaskFormer 进行分类
        maskformer_outputs = self.classifier(reduced_maskformer_inputs).encoder_last_hidden_state# 需要将数据形状调整为批次输入
        # 将 maskformer_outputs 从 16x16 上采样到 512x512
        upsampled_maskformer_outputs = self.upsample(maskformer_outputs).squeeze(0)
        maskformer_vectors = upsampled_maskformer_outputs[:, voxel_coords[:, 1], voxel_coords[:, 2]].T
        class_ids_0 = self.classifier_decoder(maskformer_vectors)
