...
class BinaryFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # 将多分类标签转换为二分类标签：0 表示背景，1 表示前景
        binary_targets = (targets != 0).float()  # 0 为背景，其他为前景

        # 提取背景类别的概率（假设背景在第 0 通道）
        bg_prob = torch.softmax(inputs, dim=1)[:, 0]  # 背景类别概率

        # 计算二分类的 Focal Loss，仅关注背景概率
        BCE_loss = F.binary_cross_entropy(bg_prob, 1 - binary_targets, reduction='none')
...
#         """
#         Focal Loss 用于多分类任务，适合静态和动态物体类别一次性分割
#         alpha: 各类别的权重列表，alpha[0] 对应静态背景，其余对应动态物体类别
#         gamma: 聚焦参数，gamma 越大，网络越关注难以分类的样本
#         """
#         super(FocalLoss, self).__init__()
#         self.alpha = alpha
#         self.gamma = gamma
...
#             at = alpha_tensor.gather(0, targets)  # 提取每个点对应的 alpha 值
...
def seflowLoss(res_dict, timer=None):
    ...
    # pc0_label_est = torch.argmax(pc0_mask, dim=1)  # 输出形状为 [N]
    ...
    # 创建一个布尔掩码，表示哪些点的 label 不为 0
    # fore_mask = pc0_label != 0
    # 使用掩码来筛选出对应的数据
    pc0 = pc0.unsqueeze(0)      # 筛选出 label 不为 0 的 pc0
    est_flow = est_flow.unsqueeze(0)  # 筛选出对应的 est_flow
    pc0_mask = pc0_mask.unsqueeze(0)  # 筛选出对应的 pc0_mask# rigid loss 12445016，只计算前景点的损失
    rigid_loss = rigidLoss(pc0, pc0_mask, est_flow)
    
    # smooth loss 12445016，只计算前景点的损失
    # 确保 N 大于或等于 16384
    ...
    if N >= target_num_points:
        ...
        pc0_fore_downsampled = pc0[:, indices, :]  # 选择点云数据
        pc0_mask_fore_downsampled = pc0_mask[:, indices, :]  # 选择对应的 mask
    else:
        pc0_fore_downsampled = pc0
        pc0_mask_fore_downsampled = pc0_mask
    
    smooth_loss = SmoothLoss(pc0_fore_downsampled, pc0_mask_fore_downsampled)
