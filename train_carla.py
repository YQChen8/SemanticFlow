...
from src.dataset_carla import HDF5Dataset, collate_fn_pad
from src.trainer_carla import ModelWrapper
from dataprocess.my_carla import CARLA3D
...
@hydra.main(version_base=None, config_path="conf", config_name="config_carla")
def main(cfg):
    precheck_cfg_valid(cfg)
    pl.seed_everything(cfg.seed, workers=True)
    use_fg_inds = True
    train_dataset = CARLA3D(root_dir=cfg.train_data + 'train/', nb_points=16384, mode="train",
                              use_fg_inds=use_fg_inds)
    test_dataset = CARLA3D(root_dir=cfg.val_data + 'val/', nb_points=16384, mode="test",
                                   use_fg_inds=use_fg_inds)
    
    # train_dataset = HDF5Dataset(cfg.train_data, n_frames=cfg.num_frames, dufo=(cfg.loss_fn == 'seflowLoss'))
    # test_dataset = HDF5Dataset(cfg.val_data, n_frames=cfg.num_frames)
    ...
    if cfg.wandb_mode != "disabled":
        logger = WandbLogger(save_dir=output_dir,
                            entity="wy138148168-southern-university-of-science-technology",
