...
# def fast_ground_filter(point_cloud, percentile=0.1, z_margin=0.05):
#     """
#     使用自适应高度阈值快速判断地面点
#     Args:
#         point_cloud: [1, N, 3] 点云数据，torch.Tensor
#         percentile: 高度分位数，用于确定地面阈值（0-1之间的浮点数）
#         z_margin: 允许的高度浮动范围，用于包含一定误差的地面点
#     Returns:
#         ground_mask: [1, N] 布尔掩码，dtype=torch.bool
#     """
#     # 提取 z 轴坐标
#     z = point_cloud[:, :, 2]  # [1, N]
    
#     # 计算地面高度的分位数 (例如：最低 20% 的高度)
#     z_threshold = torch.quantile(z, percentile)
    
#     # 生成地面布尔掩码 (z 在分位数高度附近浮动范围内视为地面点)
#     ground_mask = (z <= z_threshold + z_margin)
    
#     return ground_mask
def fast_ground_filter(point_cloud, percentile=0.2, z_margin=0.2):
    """
    使用自适应高度阈值快速判断地面点
    Args:
        point_cloud: [1, N, 3] 点云数据，torch.Tensor
    ...
    return ground_mask
def ground_filter_height_threshold(point_cloud, z_threshold=-1.7):
    """
    使用高度阈值法判断地面点
    Args:
        point_cloud: [1, N, 3] 点云数据，torch.Tensor
        z_threshold: 高度阈值，小于该高度的点被认为是地面点
    Returns:
        ground_mask: [1, N] 布尔掩码，dtype=torch.bool
    """
    # 提取 z 轴坐标
    ...
    # 生成布尔掩码
    ground_mask = z < z_threshold
    
    return ground_mask
def ground_filter_adaptive(point_cloud, quantile=0.04, tolerance=0.3):
    """
    使用自适应高度估计法判断地面点
    Args:
        point_cloud: [1, N, 3] 点云数据，torch.Tensor
        quantile: 分位数，用于估计地面高度，例如 0.05 表示 5% 分位数
        tolerance: 容忍范围，即地面高度上下浮动的范围
    Returns:
        ground_mask: [1, N] 布尔掩码，dtype=torch.bool
    """
    # 提取 z 轴坐标
    ...
    # Step 1: 估算地面高度（取 z 坐标的低分位数）
    ground_height = torch.quantile(z, quantile)  # 估算地面高度

    # Step 2: 生成地面点布尔掩码
    ground_mask = (z >= ground_height - tolerance) & (z <= ground_height + tolerance)
...
def prepare_batch(pcd_dir, pose_dir, batch_size=1, device='cuda'):
    ...
    for i in range(len(pcd_files) - 1):  # 注意这里少遍历一帧
        ...
        gm0 = ground_filter_adaptive(pc0).to(pc0.device)
        gm1 = ground_filter_adaptive(pc1).to(pc0.device)
...
def main(cfg):
    # 使用示例
    pcd_dir = "/dataset/public_dataset_nas2/sustech_points/suscape_dataset/v1.0-mini-unzipped/lidar/scene-000040/lidar"
    # pcd_dir = "/dataset/public_dataset_nas/sustechpoints/2021-08-31/2021-08-31-01-51-50_preprocessed/dataset_10hz/lidar/lidar"
    pose_dir = "/dataset/public_dataset_nas2/sustech_points/suscape_dataset/v1.0-mini-unzipped/lidar_pose/scene-000040/lidar_pose"
    # pose_dir =
    ...
    for i, batch in enumerate(prepare_batch(pcd_dir, pose_dir, batch_size=1)):
        batch_test = batch[0]
        batch_test, res_dict_test = mymodel.run_model_wo_ground_data(batch_test)
        pc0 = batch_test['origin_pc0']
        pose_0to1 = cal_pose0to1(batch_test["pose0"], batch_test["pose1"])
        transform_pc0 = pc0 @ pose_0to1[:3, :3].T + pose_0to1[:3, 3]
        pose_flow = transform_pc0 - pc0
