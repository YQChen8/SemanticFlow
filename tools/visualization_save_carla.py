...
class CARLAData:
    def __init__(self, data_dir, vis_name="flow", flow_view=True):
    ...
    def __getitem__(self, idx):
        ...
        scene_id = self.files[idx].split('.')[0]
        timestamp = '000010'  # 默认时间戳，可以根据需要修改
        ...
        gm0 = data['gm0']
        gm1 = data['gm1']
        pose0 = np.eye(4)  # 默认单位矩阵，可以根据需要修改
        pose1 = np.eye(4)  # 默认单位矩阵，可以根据需要修改
        ...
        # 添加流场数据
        if self.vis_name in data:
            res[self.vis_name] = data[self.vis_name]
            
        # 添加标签数据
        if 'est_label0' in data:
            res['est_label0'] = data['est_label0']
            
        return res

def check_flow(
    data_dir: str = "/home2/chenyq/carla_scene_flow2/val/record2023_0308_1524/rm_road/SF/10",
    res_name: str = "flow",  # "flow", "flow_est"
    start_id: int = 0,
    output_dir: str = "output_carla",  # Directory to save point clouds
):
    os.makedirs(output_dir, exist_ok=True)  # Create output directory if it doesn't exist
    dataset = CARLAData(data_dir, vis_name=res_name, flow_view=True)
